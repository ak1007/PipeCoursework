...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<!-- Changed 24 Nov 2006 by David Patterson 
		 1.  When run with Java 5.0's default parser, 
	         errors are raised because attribute definitions are not the first 
	         in element definitions for the match=place, match=arc and name=place-transition
	         templates. They have now been placed correctly.
	-->
	...
	<xsl:template match="place">
		<xsl:element name="place">
			...
			<xsl:attribute name="tagged"><xsl:value-of select="tagged/value"/></xsl:attribute>     
			<xsl:call-template name="place-transition"/>
		</xsl:element>	
	</xsl:template>
	<xsl:template match="transition">
		<xsl:element name="transition">
			<xsl:attribute name="rate"><xsl:value-of select="rate/value"/></xsl:attribute>
			<xsl:attribute name="timed"><xsl:value-of select="timed/value"/></xsl:attribute>
			<xsl:attribute name="angle"><xsl:value-of select="orientation/value"/></xsl:attribute>
			<xsl:attribute name="infiniteServer"><xsl:value-of select="infiniteServer/value"/></xsl:attribute>
			<xsl:call-template name="place-transition"/>
		</xsl:element>
	</xsl:template>
